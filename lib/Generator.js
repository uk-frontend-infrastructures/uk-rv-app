...
class Generator {
    constructor(name, targetDir, frameName) {
        // 项目名称
        this.name = name;
        // 创建位置
        this.targetDir = targetDir;
        // 框架 react / vue
        this.frameName = frameName || 'vue';
    ...
    async getRepo() {
        ...
        // 过滤我们需要的模板名称
        const repos = repoList.filter(repo => repo.name.indexOf(`${this.frameName}-cli`) > -1).map(item => item.name);
